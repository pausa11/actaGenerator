...
export default function GruposList({ initialGrupos }: { initialGrupos: Group[] }) {
  const router = useRouter();
  const [grupos, setGrupos] = useState<Group[]>(initialGrupos);
  ...
  async function crearGrupo(e: React.FormEvent) {
    ...
    try {
      ...
      const newGroup: Group = { ...(await res.json()), actaCount: 0 };
